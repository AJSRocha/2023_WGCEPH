...
load("C:/PNAB/fleet_register/fleet.Rdata")
...
# Checks
summary(factor(temp$Power.of.main.engine))
...
temp %>%
   ...
   filter(REGIAO_INICIO_EFETIVA == '27.9.a' | REGIAO_FIM_EFETIVA == '27.9.a') %>%
   select(IEMBARCA) %>%
   unique
...
temp_1 =
   ...
   summarise(CAPTURA_KG = sum(CAPTURA_KG)) %>%
   filter(ESPECIE_CODFAO != 'OTH') %>%
   # acrescenta as linhas nulas
   rbind(., temp_0) %>%
   # anonimiza
   mutate(IEMBARCA = paste(substr(IEMBARCA,1,3), as.numeric(as.factor(IEMBARCA)), sep = ""))
...
temp %>%
   ...
   group_by(CNTRY, DEPARTURE_DATAHORA, RETURN_DATAHORA, 
            IEMBARCA, Power.of.main.engine, ARTE_CODFAO, 
...
temp_3 =
   ...
   rbind(., temp_2) %>%
   # anonimiza
   mutate(IEMBARCA = paste(substr(IEMBARCA,1,3), as.numeric(IEMBARCA, sep = ""))) %>%
