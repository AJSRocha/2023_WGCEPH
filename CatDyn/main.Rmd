...
```{r}
M         <- 1/52 #1/Time step
N0.ini    <-10000 #billions
P1.ini    <- 10 #thousands
P2.ini    <- 10 #thousands
k.ini     <- 1 #1/n of boats
...
dates <- c(head(occ_cat_12_ref$Data$`Artisanal-S`$time.step,1),
           P1,P2,
           tail(occ_cat_12_ref$Data$`Artisanal-S`$time.step,1))

occ.P2.ini <- catdynexp(x=occ_cat_12_ref,
                               p=2,
                               par=pars.ini,
                               dates=dates,
                               distr="aplnormal")
